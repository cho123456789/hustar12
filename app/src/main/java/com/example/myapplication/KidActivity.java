...
//        et_gend = findViewById(R.id.kid_gend);
        ...
        et_gt1.addTextChangedListener(new PhoneNumberFormattingTextWatcher());
        et_gt2.addTextChangedListener(new PhoneNumberFormattingTextWatcher());
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (success) { // 회원등록에 성공한 경우
                                ...
                                Toast.makeText(getApplicationContext(), "정보 등록에 성공하였습니다.", Toast.LENGTH_SHORT).show();
                                Intent intent = new Intent(KidActivity.this, nfc.class);
                                intent.putExtra("kid_nm", kid_nm);
                                startActivity(intent);
                            } else { // 회원등록에 실패한 경우
                                Toast.makeText(getApplicationContext(), "정보 등록에 실패하였습니다.", Toast.LENGTH_SHORT).show();
                                return;
...
//                        parameters.put("kid_gend", et_gend.getText().toString());
                    ...
                    //    parameters.put("kid_img_url", intent.getStringExtra("kid_img_url"));
